...
namespace OOP_Assignment_4
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            EvaluateGrades(firstGrade, secondGrade);
        }
        public static void EvaluateGrades(double num1, double num2)
        {
            if ((num1 >= 1 && num1 <= 100) && (num2 >= 1 && num2 <= 100))
            {
                double average = GetAverage(num1, num2);
                string gradeEquivalent = AverageEquivalent(average);
                Console.WriteLine("Your average is " + average + " which is equivalent to " + gradeEquivalent);
            }
            else
            {
