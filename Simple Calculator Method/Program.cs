...
namespace OOP_Assignment_7
{
    internal class Program
    {
        ...
        public static string Calculate(int operation, int num1, int num2)
        {
            string result = " ";
            ...
            {
                case 1:
                    result = $"ADD {num1} and {num2} the result is {Add(num1, num2)}";
                    break;
                case 2:
                    int difference = Subtract(num1, num2);
                    result = difference >= 0 ?
                        $"SUBTRATCT {num1} and {num2} the result is {difference}"
                        : "Invalid Input.First first number should be greater than second number!";
                    break;
                case 3:
                    result = $"MULTIPLY {num1} and {num2} the result is {Multiply(num1, num2)}";
                    break;
                case 4:
                    if (num2 == 0)
                    {
                        result = "Second number cannot be 0";
                        break;
                    }
                    if (num1 > num2)
                    {
                        result = $"DIVIDE {num1} and {num2} the result is {Division(num1, num2)}";
                    }
                    else
                    {
                        result = "Invalid Input. First first number should be greater than second number!";
                    }
                    break;
                default:
                    result = "Invalid Input. Please refer to the operation above";
                    break;
            }
            return result;
        }
